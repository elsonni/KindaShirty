// netlify/functions/check-promo-v2.js
'use strict';

const VERSION = 'promo-v5'; // bump this string whenever you redeploy for visibility
...
function readFM(fp) {
  // Be tolerant of stray // comments in front-matter
...
exports.handler = async (event) => {
  try {
    ...
    // Try multiple likely directories so path differences don't bite us
    const candidates = [
    ...
    if (debug) {
      ...
      return {
        ...
        body: JSON.stringify({ version: VERSION, cwd: process.cwd(), dirTried: candidates, dirUsed, filesFound: files, parsed: peek })
      };
    }
    // --------------------------------------------------------------
    ...
    // Search files for a matching code
    let promo = null;
    ...
    // Discount percent: allow number or "10%"
    ...
    console.error('check-promo-v2 error:', err);
    return {
      statusCode: 500,
      headers: { 'Content-Type': 'application/json', 'X-Function-Version': VERSION },
